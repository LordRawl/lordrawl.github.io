  ...
  $.fn.eTimer = function(options) {
    ...
    return this.each(function() {
      ...
      element.layout = function() {
        ...
        element.html('').addClass('eTimer');        
        if (config.etTitleText != '') {
          $('.eTimer').append('<div class="etTitle">' + config.etTitleText + '</div>');
        }
        ...
        element.append('<style type="text/css">.' + elClass + ' {line-height: normal; font-family: ' + config.etFontFamily + '; color: ' + config.etTextColor + '; padding: ' + config.etPaddingTB + 'px ' + config.etPaddingLR + 'px; background: ' + config.etBackground + '; border: ' + config.etBorderSize + 'px solid ' + config.etBorderColor + '; -webkit-border-radius: ' + config.etBorderRadius + 'px; -moz-border-radius: ' + config.etBorderRadius + 'px; border-radius: ' + config.etBorderRadius + 'px; -webkit-box-shadow: ' + config.etShadow + '; -moz-box-shadow: ' + config.etShadow + '; box-shadow: ' + config.etShadow + ';} .etTitle {margin-bottom: 10px; font-size: ' + config.etTitleSize + 'px;} .etUnit {display: inline-block;} .etNumber {display: inline-block; margin: 1px; text-align: center; font-family: ' + config.etNumberFontFamily + '; font-size: ' + config.etNumberSize + 'px; color: ' + config.etNumberColor + '; padding: ' + config.etNumberPaddingTB + 'px ' + config.etNumberPaddingLR + 'px; background: ' + config.etNumberBackground + '; border: ' + config.etNumberBorderSize + 'px solid ' + config.etNumberBorderColor + '; -webkit-border-radius: ' + config.etNumberBorderRadius + 'px; -moz-border-radius: ' + config.etNumberBorderRadius + 'px; border-radius: ' + config.etNumberBorderRadius + 'px; -webkit-box-shadow: ' + config.etNumberShadow + '; -moz-box-shadow: ' + config.etNumberShadow + '; box-shadow: ' + config.etNumberShadow + ';} .etSign {text-align: center; font-size: ' + (+config.etNumberSize / 2.5) + 'px;} .etSep {display: inline-block; vertical-align: top; font-size: ' + config.etNumberSize + 'px; padding: ' + (+config.etNumberPaddingTB + +config.etNumberBorderSize) + 'px 5px;} .etSep:last-of-type {display: none;}</style>').append('<style type="text/css">.etNumber {width: ' + $('.etNumber:visible').eq(0).css('width') + ';}</style>');
      };
